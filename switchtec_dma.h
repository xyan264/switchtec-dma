...
int switchtec_fabric_notify(struct dma_device *dma_dev,
			    enum switchtec_fabric_event_type event);
#endif
...
int swiechtec_fabric_get_buffers(struct dma_device *dma_dev, int buf_num,
				 struct switchtec_buffer *bufs);

struct dma_async_tx_descriptor *switchtec_fabric_dma_prep_memcpy(
		struct dma_chan *chan, u16 dst_dfid, dma_addr_t dma_dst,
		u16 src_dfid, dma_addr_t dma_src, size_t len,
		unsigned long flags);

struct dma_async_tx_descriptor *switchtec_dma_prep_rhi(
		struct dma_chan *chan, u16 peer_rhi_dfid, u16 rhi_index,
		u16 local_rhi_dfid);
